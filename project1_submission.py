...
def generate_a_valid_start_token(probability: list):
    '''
    Given probability, return a valid token not starting with ending symbols
    '''
    word = unary_random_word_generation(probability)
    while word in Ending_symbols: #unary sentence change so that no ending symbols start first
        word = unary_random_word_generation(probability)
    return word

def unary_random_sentence_generation(task_type, file_type, train_docs="train_docs"):
    C = dict()
    for i in range(300): #bug fixed, dict update has to be done manually cannt use update operation
        D = handle_file(task_type, file_type, i, train_docs)
        for j in D:
                if j in C:
                    C[j] += D[j]
                else:
...
#     while word in Ending_symbols: #unary sentence change so that no ending symbols start first
#         word = unary_random_word_generation(probability)
    sentence = generate_a_valid_start_token(probability)
    while 1:
        word = unary_random_word_generation(probability)
        if word in Ending_symbols:
            sentence += word #revised so that the ending symbol goes right after a token
            return sentence
        sentence += " " + word
...
def bigram_random_sentence_generation(task_type, file_type, train_docs="train_docs"):
    ...
    #updated starting word method
    current_word = generate_a_valid_start_token(probability_unary_model)
    ret = current_word
    while 1: #changed condition to make ending symbol follow last word
        unary = build_unary_model(d[current_word])
        current_word = unary_random_word_generation(assign_probability_unary(unary))
        if current_word in Ending_symbols:
            ret += current_word
            return ret
        ret += " " + current_word
    return ret
...
def bigarm_random_sentence_generation_second_bigram_method(task_type, file_type, train_docs="train_docs"):
    ...
    while 1: #condition changed to make ending symbol follow last token
        unary = build_unary_model(d[current_word])
        current_word = unary_random_word_generation(assign_probability_unary(unary))
        if current_word in Ending_symbols:
            ret += current_word
...
def n_ugram_random_sentence_generation(n, task_type, file_type, train_docs="train_docs"):
    for i in range(n):
        print(unary_random_sentence_generation(task_type, file_type, train_docs))
def n_bigram_random_sentence_generation(n, task_type, file_type, train_docs="train_docs"):
    for i in range(n):
        print("first method: " + bigram_random_sentence_generation(task_type,file_type, train_docs))
        print("second method: " + bigarm_random_sentence_generation_second_bigram_method(task_type,file_type)[2:])

n_bigram_random_sentence_generation(10, 'sp', 'religion')